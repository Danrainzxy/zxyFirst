...
- (id)initWithPhotoSource:(NSObject<FGalleryViewControllerDelegate>*)photoSrc type:(FGalleryShowType)type {
...
- (void)loadView
{
    ...
    _innerContainer						= [[UIView alloc] initWithFrame:CGRectMake( 0, 0, _container.frame.size.width, Screen_Height)];
...
//    }
    ...
    self.innerContainer.backgroundColor = ColorWithHexString(ColorText_333333);
    UIButton *back = [CreateView createCustomBtnWithFrame:CGRectMake(0, [[UIApplication sharedApplication] statusBarFrame].size.height, 40, 44) title:@"" image:ImageNamed(@"navi_back_white") titleColorStr:ColorText_606060 fontSize:FontSize_14 superView:self.innerContainer];
    [back addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
//    [back release];
//
    self.imageTitleLabel = [CreateView createLabelWithFrame:CGRectMake(80, [[UIApplication sharedApplication] statusBarFrame].size.height, Screen_Width - 160, 44) text:@"" fontSize:18.0 textColorStr:@"" superView:self.innerContainer];
    _imageTitleLabel.textColor = WHITE_COLOR;
    _imageTitleLabel.textAlignment = NSTextAlignmentCenter;
    
    self.picDescLabel = [CreateView createLabelWithFrame:CGRectMake(15, ScreenHeight_WithoutBottomBar - 40, Screen_Width - 30, 40) text:@"" fontSize:FontSize_16 textColorStr:@"" superView:self.innerContainer];
    _picDescLabel.textColor = WHITE_COLOR;
    _picDescLabel.textAlignment = NSTextAlignmentRight;
    
    UIButton *downloadBtn = [CreateView createCustomBtnWithFrame:CGRectMake(Screen_Width - SpaceLeftRight - 20, Screen_Height - 20 - 20, 20, 20) title:@"" image:ImageNamed(@"xz") titleColorStr:nil fontSize:FontSize_14 superView:self.innerContainer];
    [downloadBtn addTarget:self action:@selector(downloadPic) forControlEvents:UIControlEventTouchUpInside];
    
    if (_saveBtn) {
        [self.innerContainer addSubview:_saveBtn];
    }
...
- (void)downloadPic
{
    NSString * urlStr = [_photoSource photoGallery:self urlForPhotoSize:FGalleryPhotoSizeFullsize atIndex:_currentIndex];
...
- (void)positionInnerContainer
{
	...
	{//portrait
        if (!self.navigationController.navigationBarHidden) {
            innerContainerRect = CGRectMake( 0, 0, _container.frame.size.width, Screen_Height);
        }else {
            innerContainerRect = CGRectMake( 0, 0, _container.frame.size.width, Screen_Height);
        }
...
- (void)positionScroller
{
	...
	{//portrait
		...
		scrollerRect = CGRectMake( 0, barHeight, screenFrame.size.width, _innerContainer.frame.size.height - barHeight);
	}
	else
	{//landscape
...
//    [self setTitle:[NSString stringWithFormat:@"%ld/%li", (long)_currentIndex+1, (long)[_photoSource numberOfPhotosForPhotoGallery:self]]];
    _imageTitleLabel.text = [NSString stringWithFormat:@"%ld/%li", (long)_currentIndex+1, (long)[_photoSource numberOfPhotosForPhotoGallery:self]];
    if (_showType == FGalleryShowTypeWithText) {
        if ([_photoSource respondsToSelector:@selector(photoGallery:textAtIndex:)]) {
            _picDescLabel.text = [_photoSource photoGallery:self textAtIndex:_currentIndex];
        }
    }
}
